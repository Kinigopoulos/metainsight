...
object OutstandingLast2 extends PatternType with Outstand {
  override val name: String = "Outstanding Last 2"
  ...
  override def evaluate(dataScope: DataScope, results: Array[(Any, Double)]): PatternTypeResult = {
    ...
    val resultsFirst = OutstandingLast.evaluate(dataScope, first)
    val resultsSecond = OutstandingLast.evaluate(dataScope, second)
